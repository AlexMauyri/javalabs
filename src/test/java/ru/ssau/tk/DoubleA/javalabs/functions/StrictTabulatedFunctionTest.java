...
public class StrictTabulatedFunctionTest {
    ...
    void testListTabulatedStrict() {
        manualFunction = new LinkedListTabulatedFunction(
        ...
        StrictTabulatedFunction strictManualList = new StrictTabulatedFunction(manualFunction);
        StrictTabulatedFunction strictDiscreteList = new StrictTabulatedFunction(discreteFunction);
    ...
    void testArrayTabulatedStrict() {
        manualFunction = new ArrayTabulatedFunction(
                new double[]{-3, 1.5, 6, 10.5, 15},
                new double[]{9, 2.25, 36, 110.25, 225}
        );
        ...
        Iterator<Point> iterator = strictDiscreteArray.iterator();
        int i = 0;
        while (iterator.hasNext()) {
            Point point = iterator.next();
            Assertions.assertEquals(discreteFunction.getX(i++), point.x);
